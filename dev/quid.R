...
# To use this function you need three vectors containing 
...
make.bf <- function(y, meanScale, effectScale, prep = prep.1, iter = 10000, burnin = 1000)
{
  ...
  sigma2.est <- mean(mcmc.full[keep, "sig2"])
  # i.g3 <- 2 * prep$I + 6
  # new.pop.sd <- mean(mcmc.full[keep, i.g3])
  return(list(ind.effects = m
              , posterior.mean = new.mean, posterior.sd = new.sd, posterior.sigma = sqrt(sigma2.est)
              , gamma = new.sd / sqrt(sigma2.est)
...
quid <- function(id #vector with participant ID, can be a factor or numeric
  ...
  dat <- data.frame(sub, fcond, rt)
  dat <- dat[order(dat$sub), ]
  
  # get design matrix
  prep.1 <- prep.models(id = dat$sub, fcond = dat$fcond, expect = expect)
  
  # get bayes factors
  make.bf(y = dat$rt, meanScale = prior[1], effectScale = prior[2], prep = prep.1, iter = iter, burnin = burnin)
}
