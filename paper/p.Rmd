...
## R-Package
The R-package `acdcquery` accompanying this publication allows a more customized, but still user-friendly interaction with the database. The `add_argument()`function allows easy specification of filter arguments. Any variable present in any table in the database may be used in the query. Users may also provide multiple arguments and control which of them should be combined using _AND_ and which will be combined using _OR_. Users may also request any variable present in the database to be returned by the query. 

The package constructs one SQL query combining all query arguments and returning the requested variables. This query will join multiple tables together if the user requests variables from multiple tables or the query is based on variables present in multiple tables. The query constructed by package is optimized for speed by automatically discarding unnecessary variables and eliminating the need for temporary storage of query results. All arguments and requests are combined simultaneously, allowing optimal use of SQL's base optimization.

## Queries and Output
Below, we will illustrate several examples for using the database. Before querying, the package needs to be installed from CRAN and loaded. Querying also requires that you download the database and establish a connection using `connect_to_db()`.
...
argument_list <- add_argument(
  ...
  variable = "n_participants", # The variable name. No need to specify the table
...
# same numbers lead to combination via OR, different numbers combination via AND
...
mostly_trial_targets <- c("task_name", "dataset_id", "subject", "block", "trial", "rt", "accuracy")

# Mostly higher-level data is of interest
mostly_data_targets <- c("authors", "task_name", "n_participants", "mean_dataset_rt")
...
Figure \@ref(fig:line-plot)A. shows the ratio of signal and noise standard deviations, $\gamma$ for all data sets ordered from smallest to largest. The median of $\gamma$ is $Med = `r round(median(sqrt(stn[, 1])), 2)`$ corresponding to a ratio of roughly 1 to 8.5. That is, for every unit of signal there are 8.5 units of noise. 
...
stub_indents = list(1, 2:3, 4:7, 8:12, 13:15, 16:36, 37:40, 41:48, 49:50, 51, 52:54)
