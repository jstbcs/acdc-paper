...
* SQL databases are composed of several data tables consisting of rows and columns. 
  * Each row in a data table has primary key (essentially a row ID) which uniquely identifies it. Additionally, SQL data tables may contain foreign keys which reference a unique row in another data table. In contrast to primary keys, these foreign keys allow for duplicate values within the same data table.
  * For instance, a study table may store information about all studies in a database where each row corresponds to one study. Here the primary key is the study_id. We can ensure that our database links each study to the publication it was published in by adding a foreign key called publication_id which references the unique identifier of the respective publication in a publication table (see Figure 1). 
